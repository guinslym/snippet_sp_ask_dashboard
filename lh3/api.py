...
class LH3AuthError(Exception):
    ...
    def __init__(self):
        self.message = (
            ("\n\n")
            + ("*" * 40)
            + "\n\nPlease add your LibraryH3lp username and password in the "
            + Style.BRIGHT
            + Fore.RED
            + ".secrets"
            + Fore.RESET
            + " found in the current directory:\n\t\t {0} to authenticate on the libraryh3lp servers\n".format(
                os.path.join(os.path.realpath("."), ".secrets ")
            )
            + (
...
class LibraryH3lpTimeoutIssueError(Exception):
...
class Client(object):
    ...
    def api(self, config=None):
        ...
        if CONNECTION_ISSUE:
            return False
            raise LibraryH3lpTimeoutIssueError
        return self._api
