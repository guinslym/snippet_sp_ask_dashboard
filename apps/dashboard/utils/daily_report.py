...
def save_unanswered_chat_into_db():
    today = datetime.now()
    found_object = UnansweredChat.objects.filter(
        started__month=today.month, started_year=today.year
    )
    if found_object:
...
def save_daily_report_into_db(df):
    today = datetime.now()
    found_object = ReportMonthly.objects.filter(
        started__month=today.month, started_year=today.year
    )
    if found_object:
...
def resize_column():
    ...
    dark_orange_fill = PatternFill(
        start_color="00FFCC00", end_color="00FFCC00", fill_type="solid"
    )
    light_blue_fill = PatternFill(
        start_color="00CCFFFF", end_color="00CCFFFF", fill_type="solid"
    )
    light_yellow_fill = PatternFill(
        start_color="00FFFF99", end_color="00FFFF99", fill_type="solid"
    )
