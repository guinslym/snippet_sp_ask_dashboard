        ...
        try:
            ...
            for question in info.get_list():
                ...
                if question.get("question", None):
                    ...
                    answer = (
                        client.one("faqs", search_value)
                        .one("questions", id=this_id)
                        .get()
                        .get("answer")
                    )
            ...
            if len(question_response) == 0:
                messages.add_message(
                    self.request,
                    messages.WARNING,
                    "This FAQ doesn't contain any questions",
                )
            return context
        except:
            messages.add_message(
                self.request,
                messages.WARNING,
                "An error occured. Please verify the FAQ ID",
            )
            context["faqs"] = client.all("faqs").get_list()
            return context
